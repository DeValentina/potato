...
public class ControlEx {
	public static void main(String[] args) {
		...
		while(true) {
			...
			menu = Integer.parseInt(tmp);//문자를 숫자처리 해주기 위해
			if(menu==0) {
		...
		for(;;) {//for(;;)는 무한루프
			System.out.println("계산할 값을 입력하세요.(계산 종료: 0)");
			tmp = scanner.next();
			num = Integer.parseInt(tmp);
			if(num==0)
				break;
			if(num==99) break outer; //전체 종료. 바깥쪽 while문까지 벗어남
			switch (menu) {
			case 1 :
			...
			case 2 :
				...
				break;
			case 3 :
				System.out.println("result="+Math.log(num));//제곱근
				break;
				}
			}//end for(;;)
